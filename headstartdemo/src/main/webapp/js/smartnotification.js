...
(function ($) {
    $.SmartMessageBox = function (settings, callback) {
        ...
        if (ExistMsg == 0) {
            ...
            $(".main-content").append(SmartMSG);
            if (isIE8orlower() == 1) {
                
                $("#MsgBoxBack").addClass("modal-backdrop");
                $(".divMessageBox").css("z-index",100);
               
                $(".divMessageBox").css("background-color","rgba(0,0,0,.6)");
            }
        }
        var InputType = "";
        var HasInput = 0;
                ...
                if (settings.buttons[i] == "]") {
                   ...
                   if(Name=="Cancel"){
                    Name = "<button id='bot" + NumBottons + "-Msg" + SmartMSGboxCount + "' class='btn btn-default btn-sm botTempo'> " + Name + "</button>";
						  ...
						  Name = "<button id='bot" + NumBottons + "-Msg" + SmartMSGboxCount + "' class='btn btn-warning btn-sm botTempo'> " + Name + "</button>";
						}
                    Content += Name
                } else {
